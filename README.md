...
- **스피너** — 그 탭에서 명령이 돌아가는 중이면 서브탭·메인탭에 **원형 스피너**가, Claude Code 가 작업 중이면 **Claude 로고 모양(✳) 스피너**가 돈다. 프롬프트가 돌아오면 사라진다.
...
상단 오른쪽에 **한국 · 홍콩 · 미국 동부** 시간이 `MM/DD HH:mm` 으로 표시된다.
...
1. 배포본이 GitHub Releases 에 `latest.yml`(win) / `latest-mac.yml`(mac) 과 함께 올라가 있어야 한다 → 태그를 밀면 CI 가 알아서 올린다.
   ...
   ```
2. 설치해서 쓰는 형태여야 한다. **NSIS 설치본(.exe)** 또는 mac **dmg/zip** 은 자동 설치까지 되고, 압축본을 직접 풀어 쓰는 경우에는 새 버전 알림까지만 되고 릴리스 페이지를 열어 준다.
...
상단 오른쪽 **도움 ▾** 버튼(또는 메뉴 막대의 `도움`)에서 **tmux 사용법** 과 **단축키 모음** 을 볼 수 있다.
