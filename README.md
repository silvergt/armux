...
- 주소창(입력하면 URL 이면 이동, 아니면 구글 검색), 뒤로/앞으로/새로고침/홈, ☆ 북마크 추가, 북마크 바를 갖췄다.
- **북마크 바에는 이 PC 크롬의 북마크 바를 읽어와 함께 보여 준다** (`Bookmarks` 파일을 읽는다. 크롬/엣지/크로미움, Default·Profile 1~2 순으로 탐색).
...
제목 줄을 따로 두지 않고, **맨 윗줄 하나에 아이콘 + 앱 이름 + 메뉴(탭 · 편집 · 보기 · 정보 · 도움)** 를 모았다.
윈도우/리눅스에서는 최소화·최대화·닫기 버튼만 그 줄 오른쪽에 겹쳐 표시되고, macOS 는 신호등 버튼이 왼쪽에 들어간다.
