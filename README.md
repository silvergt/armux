...
## Claude 상태 표시 (스피너 / 초록 느낌표)

탭에는 Claude 상태가 세 가지로 표시된다: **스피너**(작업 중) · **초록 느낌표**(입력 대기 또는 안 보는 탭에서 완료) · **점**(그 외, 연결 상태).

정확도를 위해 **Claude Code 훅**을 쓴다. 서버에 처음 접속하면 그 서버의 `~/.claude/settings.json` 에 훅을 **비파괴적으로 병합**한다(기존 설정·훅 보존, 처음 한 번 `.armux-backup` 백업). 훅은 Claude 가 작업 시작(`UserPromptSubmit`)·완료(`Stop`)·입력 대기(`Notification`) 때 터미널로 신호(OSC 6789)를 보내고, 앱이 그 신호로 상태를 정확히 판단한다.

- 훅은 Claude 가 **다음에 시작될 때부터** 적용된다(Claude 는 시작 시 설정을 읽는다). 이미 실행 중이던 Claude 는 재시작해야 한다.
- 훅이 아직 없는 세션에서는 화면 맨 아래 라이브 상태줄(`✻ … esc to interrupt`)을 보고 추정하는 폴백을 쓴다. 대화 본문에 그 문구가 있어도, 줄 맨 앞 스피너 글리프가 없으면 무시한다.
