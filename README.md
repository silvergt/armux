...
이 방식이라 **창을 전환하든 tmux 를 빠져나가든 다음 틱에 저절로 맞춰진다.** 예전처럼 "바뀌는 순간" 의 신호 하나에 기대지 않으므로, 무엇을 놓쳐도 틀린 상태가 2초 넘게 남지 않는다. 포그라운드 명령은 `/proc` 을 직접 읽어 알아내며(없는 서버는 `ps` 로 폴백), 한 틱에 8ms 남짓 — 접속당 CPU 0.4% 정도다.
...
| 감싸는 명령 (`sudo -i`, `sudo su`) | 뒤에 오는 것이 셸이면 놀고 있음, 아니면 작업 중 |
...
- **중첩 `tmux`·중첩 `ssh` 안쪽** — 바깥에서는 `tmux`/`ssh` 로만 보여서 판단하지 않는다(스피너가 안 뜰 뿐, 틀린 알림은 없다).
- **`git log` 처럼 페이저를 띄우는 명령** — 페이저를 읽는 동안 스피너가 켜져 있다.
- `ps` 도 `/proc` 도 쓸 수 없는 서버에서는 관찰기가 놀고, Claude 표시만 훅으로 동작한다.
