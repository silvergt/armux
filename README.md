...
연결이 끊긴 탭에서 `Enter` 를 누르면 같은 정보로 재접속한다. (설정에서 "연결이 끊기면 자동으로 다시 붙기" 를 켜면 Enter 없이 스스로 붙는다 — 기본은 꺼져 있다.)
...
**위층 — 에이전트(Claude Code · Codex).** 폴링으로는 에이전트가 **생각 중인지 입력을 기다리는지**가 똑같이 `claude`/`codex` 로만 보인다. 그래서 에이전트 창은 **화면에 작업 상태줄이 있는가**로 판정한다 — 이것도 "지금 상태 그 자체" 라 놓칠 게 없다.

- 보이는 창은 우리 xterm 화면을 250ms 마다 본다.
- 안 보이는 tmux 창은 관찰기가 `tmux capture-pane` 으로 밑 15줄을 읽어 온다(창당 3ms).
- 상태줄 형식: 요즘 Claude `✶ Working… (2s · ↓ 16 tokens)`, 예전 Claude `✻ Crunching… (esc to interrupt)`, Codex `• Working (3s • esc to interrupt)`. 공통점은 **맨 앞 스피너 글리프 + 괄호 안의 경과 초(또는 esc to interrupt)** 다. 끝난 뒤의 `✻ Crunched for 14s · done` 은 괄호가 없어 안 걸린다.

**상태줄 문구와 무관한 안전망도 있다.** 에이전트가 형식을 또 바꿔서 상태줄을 못 알아보더라도, 화면 본문(밑 3줄 제외)의 지문이 **연속으로 두 번** 바뀌면 작업 중으로 본다. 작업 중이면 초 카운터 때문에 매초 바뀌고, 놀 때는 입력창 안내 문구가 20초에 한 번쯤 바뀔 뿐이라 한 번 바뀐 것으로는 작업이라 보지 않는다(실측: 놀 때 60초에 3번, 작업 중 2초마다 매번, 질문 대기 중 30초에 1번). 상태줄 인식을 일부러 꺼 놓고 돌려도 스피너가 켜지고 ESC 로 끊으면 3초 안에 꺼지는 것을 확인했다.

훅(OSC 6789)은 **빠른 길**로만 쓴다. 프롬프트를 보낸 직후 화면이 그려지기 전 5초는 `busy` 훅을 믿고, 그 뒤로는 화면이 정한다. `idle`(Stop) 훅은 오면 곧바로 반영하고 알린다. **훅에만 기대면 안 되는 이유**: Stop 훅은 사용자가 ESC 로 끊으면 오지 않는다(Claude Code 사양). 그래서 훅만 믿던 때는 ESC 뒤 스피너가 영영 남았다. 화면을 보면 5초 안에 꺼진다.

에이전트가 작업 중(화면에 상태줄)일 때는 옛 화면 문구 규칙(`1. Yes`, `Do you want …?`)을 보지 않는다. Claude 가 답변 본문에 그런 말을 쓰면 진짜 질문인 줄 알고 느낌표를 올렸기 때문이다. 진짜 권한 질문이 뜰 때는 상태줄이 사라지므로 그때는 평소대로 본다(그리고 훅이 더 먼저 알려 준다).
...
| 에이전트가 있는 창 (`claude` `codex`) | **화면의 작업 상태줄이 정한다.** 훅은 시작 직후 5초와 완료 알림에만 |
...
- Claude 가 완료 신호를 못 보내고 죽어도(크래시·`kill`), ESC 로 끊어도 스피너가 남지 않는다. 화면이 조용해지면 낡은 훅 상태를 지운다.
