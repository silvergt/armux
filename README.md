...
**위층 — 에이전트(Claude Code · Codex) 신호.** 폴링으로는 에이전트가 **생각 중인지 입력을 기다리는지**가 똑같이 `claude`/`codex` 로만 보인다. 그 구간만 훅(OSC 6789)이 정하고, 위층이 이긴다. 신호에는 **tmux pane 이름표**(`%3`)가 함께 실려 오므로 창마다 따로 기억한다.

Claude 는 시작·완료·입력 대기를 모두 알려 주지만, **Codex 는 "시작" 이벤트가 없다** — `agent-turn-complete`(완료)와 승인 대기만 보낸다. 그래서 Codex 가 생각 중인지는 화면의 작업 상태줄(`◦ Working (3s • esc to interrupt)`)로 본다. 화면은 지금 보이는 창의 것이라, 안 보이는 tmux 창의 Codex 는 스피너 없이 **완료될 때 초록 느낌표만** 뜬다. 완료 신호는 시작을 못 봤어도 그 자체로 알림이 된다.
...
| 에이전트가 있는 창 (`claude` `codex`) | **훅이 정한다.** Codex 는 "생각 중" 만 화면으로 본다 |
