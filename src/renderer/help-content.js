...
bind C-a send-prefix</pre>
  ...
  shortcuts: {
    ...
    html: `
      <h3>설정</h3>
      <p>
        상단 <b>정보 ▸ 설정</b>(맥은 메뉴 막대의 <b>Armux Terminal ▸ 설정…</b> 또는 <code>⌘,</code>)에서 연다.
        <b>모양</b>(화면 색, 글자 크기·글꼴, 커서, 스크롤백) · <b>단축키</b> · <b>동작</b> 세 갈래다.
      </p>
      <p>
        <b>화면 색</b>을 밝게로 두면 터미널과 앱 화면 전체(탭바·상태바·메모·파일·AI 채팅·설정 등)가
        밝아지고, Claude Code·Codex 의 라이트 테마도 제대로 보인다.
        그 도구들은 자기가 칠하는 글자 색만 바꿀 뿐 판 배경은 못 바꾼다(배경은 터미널 소유다).
      </p>
      <ul>
        <li><b>메인탭 ↔ 서브탭 번호 바꾸기</b> — 기본은 <code>Ctrl/⌘+숫자</code> 가 서브탭,
            <code>Ctrl+Alt/⌘⌃+숫자</code> 가 메인탭인데 설정에서 맞바꿀 수 있다.</li>
        <li>목록의 단축키는 눌러서 새 키를 지정하고, ↺ 로 기본값으로 되돌린다.
            복사·붙여넣기처럼 터미널 입력과 얽힌 키는 바꿀 수 없다.</li>
        <li><b>연결이 끊기면 자동으로 다시 붙기</b> — 기본은 <b>꺼짐</b>. 켜면 판을 죽이지 않고
            기다렸다가 스스로 다시 붙는다. 다만 다시 붙는 것은 새 셸이라, 하던 작업이 tmux
            안이 아니었다면 이어지지 않는다. 꺼져 있어도 잠깐 끊긴 정도는 SSH keepalive 가
            버텨 주므로 애초에 끊기지 않는다.</li>
        <li><b>접속 시도 중단</b> — "접속 중…" 이거나 자동 재접속을 기다리는 판에서
            <code>Ctrl+C</code>(맥은 <code>⌘C</code> 도)를 누르면 시도를 끊고
            "Enter 를 누르면 다시 접속합니다" 상태로 돌아간다.</li>
      </ul>

      <h3>뽀모도로 타이머</h3>
      <p>
        서브탭 줄 오른쪽 끝(상단 시계 바로 아래)에 있다.
        <b>▶</b> 로 시작하고 <b>■</b> 로 멈춘다 — 멈춰도 남은 시간은 그대로라 이어서 갈 수 있다.
        <b>창을 누르면 시간을 분 단위로 직접 입력</b>할 수 있고, 5·15·25·50분 단추도 있다.
        다 되면 알림이 뜨고 맞춰 둔 시간으로 되돌아간다. 고른 시간은 다음 실행에도 유지된다.
      </p>
